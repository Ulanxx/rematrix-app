...
<body>
   ...
   * 获取列表项图标
   */
  private getBulletIcon(index: number, theme: string): string {
    const iconSets = {
      modern: [
    ...
    const icons = iconSets[theme] || iconSets.modern;
  ...
  private autoAssignLayout(
    ...
    // 第二页使用目录（如果有足够内容）
    if (slideIndex === 1 && totalSlides > 3) {
      return 'toc';
    }

    // 倒数第二页使用总结
    if (slideIndex === totalSlides - 2 && totalSlides > 4) {
      return 'summary';
    }
  ...
  private selectLayoutByContent(
    ...
    // 对比内容
    if (analysis.hasComparison || analysis.bulletCount === 2) {
      return 'comparison';
    }

    // 步骤或流程内容
    if (analysis.hasSteps && analysis.bulletCount >= 3) {
      return 'cards';
    }

    // 引用内容
    if (analysis.hasQuotes && analysis.bulletCount <= 2) {
      return 'quote';
    }

    // 大量要点使用卡片布局
    if (
      analysis.bulletCount > 6 ||
      (analysis.bulletCount > 4 && analysis.hasIcons)
    ) {
      return 'cards';
    }
    ...
    // 双栏内容
    if (
      analysis.bulletCount === 2 ||
      (analysis.bulletCount === 4 && !analysis.hasImages)
    ) {
      return 'two-column';
    }
    ...
    // 标题较重的内容
    if (analysis.isTitleHeavy && analysis.bulletCount <= 3) {
      return 'content';
    }
   ...
   * 检查是否包含对比关键词
   */
  private hasComparisonKeywords(content: string): boolean {
   ...
   * 检查是否包含步骤关键词
   */
  private hasStepKeywords(content: string): boolean {
    const stepKeywords = [
      '步骤',
      '流程',
      '过程',
    ...
    return stepKeywords.some((keyword) => content.includes(keyword));
  }

  /**
   * 检查是否包含引用关键词
   */
  private hasQuoteKeywords(content: string): boolean {
    const quoteKeywords = [
      '引用',
      '名言',
      '格言',
    ...
    return quoteKeywords.some((keyword) => content.includes(keyword));
  }

  /**
   * 检查是否包含列表关键词
   */
  private hasListKeywords(content: string): boolean {
    const listKeywords = [
      '列表',
      '清单',
      '目录',
   ...
   * 获取推荐的布局类型列表
   */
  private getRecommendedLayouts(slideData: PptSlideData): PageLayoutType[] {
    const analysis = this.analyzeSlideContent(slideData);
    const recommendations: PageLayoutType[] = [];

    // 基于内容特征添加推荐
    if (analysis.hasImages) {
      recommendations.push('image-text');
    }
    ...
    if (analysis.bulletCount > 4) {
      recommendations.push('cards');
    }

    if (analysis.bulletCount === 2) {
      recommendations.push('two-column');
    }
    ...
    // 总是包含基础布局
    if (!recommendations.includes('content')) {
   ...
   * 智能布局优化
   */
  private optimizeLayoutAssignment(slidesData: PptSlideData[]): PptSlideData[] {
    const optimizedSlides = [...slidesData];
    ...
    optimizedSlides.forEach((slide, index) => {
      ...
      // 如果当前布局使用过于频繁，尝试使用推荐布局
      if (layoutDistribution[currentLayout] > optimizedSlides.length / 3) {
        const alternativeLayout = recommendedLayouts.find(
          (layout) =>
            layout !== currentLayout &&
            layoutDistribution[layout] < optimizedSlides.length / 4,
        );

        if (alternativeLayout) {
          slide.design.layout = alternativeLayout;
          layoutDistribution[currentLayout]--;
          layoutDistribution[alternativeLayout]++;
        }
      }
    });
   ...
   * 分析布局分布
   */
  private analyzeLayoutDistribution(
    slidesData: PptSlideData[],
  ): Record<PageLayoutType, number> {
    const distribution: Record<string, number> = {};

    slidesData.forEach((slide) => {
      const layout = slide.design.layout;
      distribution[layout] = (distribution[layout] || 0) + 1;
    });

    return distribution as Record<PageLayoutType, number>;
  }
